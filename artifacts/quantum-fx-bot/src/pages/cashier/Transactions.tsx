...
export default function Transactions() {
  ...
  const { data: transactions, isLoading } = useListTransactions({ type: filter });
        ...
        <div className="space-y-3">
          {isLoading ? (
              ...
              <div key={tx.id} className="flex items-center justify-between p-4 bg-card rounded-2xl">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    ...
                    {tx.type === 'deposit' ? <ArrowDownRight className="w-5 h-5 text-green-500" /> : 
                     tx.type === 'withdrawal' ? <ArrowUpRight className="w-5 h-5 text-red-500" /> :
                     <Zap className="w-4 h-4 text-primary fill-primary" />}
                  </div>
                  <div>
                    <div className="font-semibold text-sm mb-0.5 capitalize">{tx.type}</div>
                    <div className="text-xs text-muted-foreground">{new Date(tx.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                  </div>
                </div>
            ...
            <div className="text-center py-16 px-4 bg-card rounded-2xl flex flex-col items-center justify-center">
              ...
              <h3 className="font-bold text-lg mb-1">No transactions yet</h3>
