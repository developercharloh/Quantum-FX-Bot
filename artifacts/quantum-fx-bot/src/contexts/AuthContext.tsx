...
export function AuthProvider({ children }: { children: ReactNode }) {
  const [token, setToken] = useState<string | null>(() => {
    return localStorage.getItem("qfx_token");
  });
  ...
  useEffect(() => {
    setAuthTokenGetter(() => localStorage.getItem("qfx_token"));
  }, []);
  ...
  useEffect(() => {
    ...
    if (meData) {
      ...
      setUser(null);
      localStorage.removeItem("qfx_token");
      setToken(null);
      setIsInitializing(false);
    }
  ...
  const setAuth = (newToken: string, newUser: User) => {
    localStorage.setItem("qfx_token", newToken);
    setAuthTokenGetter(() => localStorage.getItem("qfx_token"));
    setToken(newToken);
    setUser(newUser);
    setIsInitializing(false);
